...
def field_is_set(field: str, db: TextIO, output: TextIO) -> None:
    for entity in get_entities(db):
...
def field_is_arg(field: str, arg, db: TextIO, output: TextIO) -> None:
    for entity in get_entities(db):
        if str(getattr(entity, field)) == arg:
            print(entity, file=output)
        ...
        with open(db_name, 'r') as db:
            ...
            try:
                match cond, args[0]:
                    ...
                    case 'is', 'set':
                        field_is_set(field=field, db=db, output=output)
                    case 'is', _:
